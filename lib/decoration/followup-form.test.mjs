...
const valid = { date: '2026-07-17', nextDate: '2026-07-20', note: ' Call customer ', closeInquiry: false };
test('normalizes follow-up payload', () => assert.deepEqual(buildDecorationFollowupPayload(valid), { date: '2026-07-17', nextDate: '2026-07-20', note: 'Call customer', closeInquiry: false }));
test('includes atomic inquiry closure when selected', () => assert.equal(buildDecorationFollowupPayload({ ...valid, closeInquiry: true }).closeInquiry, true));
