...
test('View uses a static query-string destination while Download remains binary', async () => {
  ...
  assert.doesNotMatch(source, /action\.id === 'share'/);
  assert.doesNotMatch(source, /createPdfShareController/);
...
test('Download prevents duplicates and exposes busy and inline error states', async () => {
  ...
  assert.doesNotMatch(source, /Preparing share…/);
