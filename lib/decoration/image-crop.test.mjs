...
function makeAdapters({ transparent = false, blob = new Blob(['crop']), decodeError, width = 800, height = 600 } = {}) {
  const events = [];
  const canvases = [];
  const adapters = {
    async decodeBitmap(_file, options) {
      ...
      return { width, height, close() { events.push('bitmap-close'); } };
    },
...
test('exports the complete image within 2400 pixels without cropping or upscaling', async () => {
  const large = makeAdapters({ width: 4000, height: 2000 });
  const largeResult = await exportDecorationFullImage(
    new File(['source'], 'wide.png', { type: 'image/png' }),
    large.adapters,
  );
  assert.deepEqual(
    [large.canvases[0].width, large.canvases[0].height],
    [0, 0],
  );
  assert.deepEqual(
    large.events.find(
      (event) => Array.isArray(event) && event[0] === 'output:draw',
    ),
    ['output:draw', 0, 0, 2400, 1200],
  );
  assert.equal(largeResult.name, 'wide-full.jpg');
  assert.equal(largeResult.type, 'image/jpeg');
  assert.ok(large.events.includes('blob:image/jpeg:0.88'));
  assert.deepEqual(large.events.slice(-2), ['bitmap-close', 'output:release']);

  const small = makeAdapters({ width: 640, height: 480 });
  await exportDecorationFullImage(
    new File(['source'], 'small.jpg', { type: 'image/jpeg' }),
    small.adapters,
  );
  assert.deepEqual(
    small.events.find(
...
test('keeps full-image transparency as PNG with a matching filename', async () => {
  const { adapters, events } = makeAdapters({
    transparent: true,
    width: 1200,
    height: 2000,
  });
  const result = await exportDecorationFullImage(
    new File(['source'], 'overlay.webp', { type: 'image/webp' }),
    adapters,
  );
  assert.equal(result.name, 'overlay-full.png');
