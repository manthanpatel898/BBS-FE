...
const pageUrl = new URL('../../app/(app)/decoration/print/page.tsx', import.meta.url);
...
test('customer document consumes the normalized DTO and renders optional company branding', async () => {
  ...
  assert.match(page, /DecorationCustomerDocumentView document={document}/);
...
test('print route remains query-only and preserves booking Back navigation', async () => {
  ...
  assert.match(source, /href={decorationEventsUrl\(\{ date: returnDate \|\| document\.event\.startDate\.slice\(0, 10\), bookingId: document\.booking\.id \}\)}/);
