...
test('print viewer restores the popup through static query parameters', async () => {
  const source = await readFile(
    new URL('../../app/decoration/print/page.tsx', import.meta.url),
    'utf8',
  );
  assert.match(source, /params\.get\('returnDate'\)/);
  assert.match(source, /decorationEventsUrl\(\{ date: returnDate \|\| null, bookingId \}\)/);
});
