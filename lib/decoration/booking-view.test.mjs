...
test('uses only the event start date for legacy multi-day records', () => {
  ...
  assert.equal(bookingIntersectsDate(booking, '2026-07-18'), false);
  assert.equal(bookingIntersectsDate(booking, '2026-07-19'), false);
...
test('provides readable metadata for every decoration booking status', () => {
  ...
  for (const status of statuses) {
    ...
    assert.match(metadata.cardClass, /border-/);
  }
});

test('uses banquet card colors for equivalent decoration statuses', () => {
  assert.match(getDecorationStatusMeta('INQUIRY').cardClass, /border-amber-300 bg-amber-50\/60/);
  assert.match(getDecorationStatusMeta('CONFIRMED').cardClass, /border-emerald-300 bg-emerald-50\/60/);
  assert.match(getDecorationStatusMeta('DECORATION_SELECTED').cardClass, /border-emerald-300 bg-emerald-50\/60/);
  assert.match(getDecorationStatusMeta('COMPLETED').cardClass, /border-emerald-300 bg-emerald-50\/60/);
  assert.match(getDecorationStatusMeta('CLOSED_INQUIRY').cardClass, /border-slate-400 bg-slate-100/);
  assert.match(getDecorationStatusMeta('CANCELLED').cardClass, /border-red-300 bg-red-50\/60/);
});
