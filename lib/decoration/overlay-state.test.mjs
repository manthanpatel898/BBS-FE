...
test('derives the retained calendar, day, detail, and child layers', () => {
  const day = decorationOverlayReducer(initialDecorationOverlayState, { type: 'OPEN_DAY', date: '2026-07-17', origin: 'EVENTS' });
  const detail = decorationOverlayReducer(day, { type: 'OPEN_DETAIL', bookingId: 'booking-1' });
  const child = decorationOverlayReducer(detail, { type: 'OPEN_CHILD', child: 'EDIT' });
  assert.equal(getDecorationOverlayLayer(initialDecorationOverlayState), 'CALENDAR');
  assert.equal(getDecorationOverlayLayer(day), 'DAY');
  assert.equal(getDecorationOverlayLayer(detail), 'DETAIL');
  assert.equal(getDecorationOverlayLayer(child), 'DETAIL_CHILD');
});

test('restores query-owned day and detail while closing any child workflow', () => {
  const restored = decorationOverlayReducer(
    { date: '2026-07-17', bookingId: 'old', child: 'ADVANCE', origin: 'DASHBOARD' },
    { type: 'RESTORE_QUERY', date: '2026-07-18', bookingId: 'booking-2' },
  );
