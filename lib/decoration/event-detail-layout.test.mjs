...
test('customer card consumes available width instead of the generic four-column grid', async () => {
  const source = await readFile(detailUrl, 'utf8');

  assert.match(source, /title="Customer" compact><Grid variant="customer"/);
  assert.match(source, /variant === 'customer'/);
  assert.match(source, /grid-cols-1 min-\[420px\]:grid-cols-2 lg:grid-cols-1/);
});
