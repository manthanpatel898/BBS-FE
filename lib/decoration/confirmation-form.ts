...
export function validateDecorationConfirmationForm(form: DecorationConfirmationForm, packageRate: number, totalCollected: number) {
  ...
  else if (amount > outstanding) errors.advanceAmount = `Advance amount cannot exceed ₹${outstanding.toLocaleString('en-IN')}`;
