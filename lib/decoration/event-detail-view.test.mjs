...
test('shows edit selection and customer document actions when a confirmed booking has a snapshot', () => {
  const value = booking('DECORATION_SELECTED', [{ itemName: 'Sofa' }]);
  assert.deepEqual(ids(value), ['edit', 'advance', 'followup', 'edit-decoration', 'view', 'download', 'print']);
  assert.equal(canShowCustomerDocument(value), true);
});

test('keeps cancelled and closed bookings read-only', () => {
  assert.deepEqual(ids(booking('CANCELLED', [{ itemName: 'Sofa' }])), ['view', 'download', 'print']);
