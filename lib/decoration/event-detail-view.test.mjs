...
const all = { canEdit: true, canConfirm: true, canAddPayment: true, canManageFollowups: true, canSelectDecoration: true, canPrint: true, canDelete: false };
...
test('allows inquiry decoration selection without advance', () => {
  assert.deepEqual(ids(booking('INQUIRY')), ['edit', 'followup', 'confirm', 'choose-decoration']);
  assert.equal(canShowCustomerDocument(booking('INQUIRY')), false);
});

test('shows edit, view, and download after an inquiry selects decoration', () => {
  const value = booking('INQUIRY', [{ itemName: 'Sofa' }]);
  assert.deepEqual(ids(value), ['edit', 'followup', 'confirm', 'edit-decoration', 'view', 'download']);
...
test('offers permanent deletion only when explicitly authorized', () => {
  assert.equal(ids(booking('INQUIRY')).includes('delete'), false);
  assert.equal(getDecorationDetailActions(booking('INQUIRY'), { ...all, canDelete: true }).at(-1)?.id, 'delete');
});
