...
const all = { canEdit: true, canConfirm: true, canAddPayment: true, canManageFollowups: true, canSelectDecoration: true, canPrint: true, canShare: true };
...
test('shows view, download, and capable-device share without print', () => {
  const value = booking('DECORATION_SELECTED', [{ itemName: 'Sofa' }]);
  assert.deepEqual(ids(value), ['edit', 'advance', 'followup', 'edit-decoration', 'view', 'download', 'share']);
  assert.equal(canShowCustomerDocument(value), true);
});

test('keeps cancelled and closed bookings read-only', () => {
  assert.deepEqual(ids(booking('CANCELLED', [{ itemName: 'Sofa' }])), ['view', 'download', 'share']);
  assert.deepEqual(ids(booking('CLOSED_INQUIRY')), []);
});

test('hides share when the device cannot share PDF files', () => {
  assert.deepEqual(
    getDecorationDetailActions(booking('DECORATION_SELECTED', [{ itemName: 'Sofa' }]), { ...all, canShare: false }).map((action) => action.id),
