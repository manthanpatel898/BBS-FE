...
test('normalizes supported tabs and falls back to profile', () => {
  ...
  assert.equal(normalizeDecorationSettingsTab('hotDates'), 'hotDates');
...
test('requires company name and at least one valid contact number', () => {
  assert.deepEqual(validateCompanyProfile({ name: '', contactNumbers: '', address: '' }), {
    name: 'Company name is required',
    contactNumbers: 'Add at least one contact number',
  });
  assert.deepEqual(validateCompanyProfile({ name: 'Decor Co', contactNumbers: '123', address: 'Ahmedabad' }), {
