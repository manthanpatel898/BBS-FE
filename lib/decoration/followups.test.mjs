...
test('places scheduled and new inquiries on their actionable dates', () => {
  const result = buildDecorationFollowupSchedule([
  ...
  assert.deepEqual(result.map((entry) => entry.dateKey), ['2026-07-21', '2026-07-22']);
});
...
test('required queue contains only follow-ups that need action today', () => {
  ...
  assert.deepEqual(result.map(({ dateKey }) => dateKey), ['2026-07-19', '2026-07-20', '2026-07-25']);
});
