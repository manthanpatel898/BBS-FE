...
test('places an actionable scheduled follow-up on its scheduled date', () => {
...
test('places scheduled inquiries on their scheduled date and new inquiries on today', () => {
  const result = buildDecorationFollowupSchedule([
  ...
  assert.deepEqual(result.map((entry) => entry.dateKey), ['2026-07-20', '2026-07-21']);
});

test('keeps a new inquiry visible today before its first follow-up', () => {
  const result = buildDecorationFollowupSchedule([
...
test('excludes past event dates and keeps today', () => {
  const result = buildDecorationFollowupSchedule([
  ...
  assert.deepEqual(result.map((entry) => entry.booking.id), ['today']);
});
...
test('groups scheduled follow-ups by month in chronological order', () => {
  const schedule = buildDecorationFollowupSchedule([
    booking({ id: 'aug', startDate: '2026-08-02', endDate: '2026-08-02', followups: [followup({ nextDate: '2026-08-01' })] }),
    booking({ id: 'jul', startDate: '2026-07-30', endDate: '2026-07-30', followups: [followup({ nextDate: '2026-07-29' })] }),
  ], '2026-07-20');
...
test('required queue contains only follow-ups that need action today', () => {
  const result = buildDecorationRequiredFollowupQueue([
    ...
    booking({ id: 'selected', status: 'DECORATION_SELECTED' }),
  ...
  assert.deepEqual(result.map(({ dateKey }) => dateKey), ['2026-07-19', '2026-07-20', '2026-07-20']);
});

test('does not revive an older pending follow-up after the latest one is completed', () => {
  const result = buildDecorationRequiredFollowupQueue([
    booking({ followups: [
      followup({ id: 'old', date: '2026-07-10', nextDate: '2026-07-20' }),
      followup({ id: 'complete', date: '2026-07-19', nextDate: null, status: 'COMPLETED' }),
    ] }),
  ], '2026-07-20');
  assert.deepEqual(result, []);
});
