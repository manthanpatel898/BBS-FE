...
test('event-decoration employee payloads enforce the Company Admin policy', () => {
  const update = buildEmployeeUpdatePayload(managerForm, 'EVENT_DECORATION');
  const create = buildEmployeeCreatePayload(
    { ...managerForm, password: 'Secure123' },
    'EVENT_DECORATION',
  );
  assert.equal(update.role, 'company_admin');
  assert.equal(update.designation, 'Company Admin');
  assert.equal(update.canAccessOdc, false);
  assert.deepEqual(update.permissions, []);
  assert.equal(create.role, 'company_admin');
  assert.equal(create.password, 'Secure123');
});
...
test('employee screen gates ODC controls by business type', async () => {
  ...
  assert.match(source, /const isEventCompany = user\?\.businessType === 'EVENT_DECORATION'/);
  assert.match(source, /!isEventCompany && permissionEditor/);
  assert.match(source, /!isEventCompany && signatureModal/);
  assert.match(source, /aria-label="User type"/);
  assert.match(source, /buildEmployeeUpdatePayload/);
  assert.match(source, /buildEmployeeCreatePayload/);
  assert.match(source, /fetchEmployeeUsernameAvailability/);
  assert.match(source, /Generate again/);
  assert.match(source, /aria-live="polite"/);
  assert.match(source, /isUsernameSubmissionBlocked/);
});
