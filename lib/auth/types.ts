...
export interface DecorationSnapshotLine { itemId:string|null; itemName:string; categoryId?:string|null; categoryName?:string|null; quantity:number; description:string|null; image:{key:string;url:string}|null; logisticsMode?:DecorationLogisticsMode; rangeStart?:string; rangeEnd?:string; reservedUnitIds?:string[]; isCustom:boolean; }
...
export interface DecorationDashboardData { todayEvents:number; upcoming:number; followupsDue:number; selectionPending:number; inventoryConflicts:number; maintenanceItems:number; byStatus:Record<string,number>; packageValue:number; collected:number; outstanding:number; upcomingEvents:DecorationBooking[]; followupPriorities:Array<{booking:DecorationBooking;followup:DecorationFollowup}>; }
