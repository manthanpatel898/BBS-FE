...
export interface BanquetInvoice extends BanquetInvoicePreview { id:string; restaurantId?:string; bookingId:string; invoiceNumber:string; status:'ISSUED'|'CANCELLED'; issuedAt:string; cancelledAt?:string|null; cancellationReason?:string|null; issuedByName?:string; recipientSnapshot:BanquetInvoiceRecipient; supplierSnapshot:InvoiceBillingProfile; bankSnapshot:InvoiceBankAccount; eventSnapshot:{bookingNumber:string;eventType:string;eventDate:string;startTime:string;endTime:string;serviceSlot:string;hall:string;persons:number}; tax:{mode:'CGST_SGST'|'IGST'|'NO_GST';rateBps:number;taxPaise:number;cgstPaise:number;sgstPaise:number;igstPaise:number}; totals:BanquetInvoiceTotals }
...
export interface PaginatedBanquetInvoices { records:BanquetInvoice[]; pagination:{page:number;limit:number;total:number;totalPages:number;hasPrevious:boolean;hasNext:boolean} }
export interface BanquetInvoiceSummary { issuedCount:number;cancelledCount:number;taxableSubtotalPaise:number;taxPaise:number;grandTotalPaise:number }
...
export interface DecorationSnapshotLine { itemId:string|null; itemName:string; categoryId?:string|null; categoryName?:string|null; quantity:number; position?:number; description:string|null; image:{key:string;url:string;displayMode?:DecorationImageDisplayMode}|null; logisticsMode?:DecorationLogisticsMode; rangeStart?:string; rangeEnd?:string; reservedUnitIds?:string[]; isCustom:boolean; }
export interface DecorationDraftBlock { clientId:string; position:number; kind:'CATALOG'|'CUSTOM'; itemId?:string; categoryId?:string; title:string; quantity:number; description:string|null; image:{key:string;url:string;displayMode:DecorationImageDisplayMode}; }
...
export interface DecorationBooking { id:string; bookingNumber:string; customer:{name:string;mobile:string;alternativeMobile?:string|null;address?:string|null}; eventType:{id:string;name:string}; venue:{id:string;name:string}; hall:{id:string;name:string}|null; address:string|null; functionName:string; timeSlot:'MORNING'|'AFTERNOON'|'EVENING'; startDate:string; endDate:string; startTime:string; endTime:string; packageRate:number; isPackagePriceFinalized:boolean; totalCollected:number; outstandingAmount:number; notes:string|null; decorationGeneralNotes?:string|null; status:DecorationBookingStatus; followups:DecorationFollowup[]; payments:DecorationPayment[]; decorationSnapshot?:DecorationSnapshotLine[]; createdBySnapshot?:{userId:string;name:string}; createdAt?:string; updatedAt?:string; cancellationReason?:string|null; decorationSelectedAt?:string|null; followupState?:DecorationFollowupPriorityState; }
export interface DecorationCustomerDocument { company:{name:string;logoUrl:string|null;contactNumbers:string[];address:string|null}; booking:{id:string;bookingNumber:string;status:string}; customer:{name:string;mobile:string;alternativeMobile?:string|null;address?:string|null}; financials:{isPackagePriceFinalized:boolean;finalPackageAmount:number|null;totalAmountReceived:number;pendingAmount:number|null}; payments:Array<{amount:number;mode:string;date:string;remark:string|null}>; event:{eventType:string;startDate:string;startTime:string;endTime:string;timeSlot:string;location:string;hall:string|null;address:string|null}; notes?:string|null; generalNotes?:string|null; categories:Array<{id:string;name:string;items:Array<{itemName:string;quantity:number;description:string|null;image:{key:string;url:string;displayMode?:DecorationImageDisplayMode}|null;isCustom:boolean}>}>; }
