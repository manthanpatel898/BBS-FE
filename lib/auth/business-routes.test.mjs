...
test('blocks the decoration module safely when its rollout flag is disabled', () => {
  assert.equal(
    getBusinessHomeRoute('EVENT_DECORATION', false),
    '/access-denied',
  );
  assert.equal(
    isRouteAllowedForBusiness('/decoration/dashboard', 'EVENT_DECORATION', false),
    false,
  );
  assert.equal(
    isRouteAllowedForBusiness('/access-denied', 'EVENT_DECORATION', false),
    true,
  );
});
