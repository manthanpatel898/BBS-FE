...
export async function uploadDecorationItemImage(token: string, id: string, file: File) { const body = new FormData(); body.append('file', file, file.name); const response = await fetch(`${API_URL}/decoration/catalog/items/${id}/images`, { method: 'POST', headers: { Authorization: `Bearer ${token}` }, body }); return parseResponse<DecorationItem>(response, { notifyOnUnauthorized: true }); }
