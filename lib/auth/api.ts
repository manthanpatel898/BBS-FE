...
export async function uploadDecorationItemImage(token: string, id: string, file: File, displayMode:import('./types').DecorationImageDisplayMode='COVER') { const body = new FormData(); body.append('file', file, file.name); body.append('displayMode',displayMode); const response = await fetch(`${API_URL}/decoration/catalog/items/${id}/images`, { method: 'POST', headers: { Authorization: `Bearer ${token}` }, body }); return parseResponse<DecorationItem>(response, { notifyOnUnauthorized: true }); }
export const setDecorationItemCoverImage = (token:string,id:string,imageId:string) => authorizedRequest<DecorationItem>(`/decoration/catalog/items/${encodeURIComponent(id)}/cover`,token,{method:'PATCH',body:JSON.stringify({imageId})});
export const removeDecorationItemImage = (token:string,id:string,imageId:string) => authorizedRequest<DecorationItem>(`/decoration/catalog/items/${encodeURIComponent(id)}/images/${encodeURIComponent(imageId)}`,token,{method:'DELETE'});
export const fetchDecorationBookings=(token:string)=>authorizedRequest<DecorationBooking[]>('/decoration/bookings',token);
export const fetchDecorationHotDates=(token:string,year:number)=>authorizedRequest<HotDate[]>(`/decoration/hot-dates?year=${year}`,token);
export const createDecorationHotDate=(token:string,payload:{year:number;date:string;description:string})=>authorizedRequest<HotDate>('/decoration/hot-dates',token,{method:'POST',body:JSON.stringify(payload)});
export const updateDecorationHotDate=(token:string,id:string,payload:{date?:string;description?:string})=>authorizedRequest<HotDate>(`/decoration/hot-dates/${encodeURIComponent(id)}`,token,{method:'PATCH',body:JSON.stringify(payload)});
export const deleteDecorationHotDate=(token:string,id:string)=>authorizedRequest<null>(`/decoration/hot-dates/${encodeURIComponent(id)}`,token,{method:'DELETE'});
export async function bulkUploadDecorationHotDates(token:string,year:number,file:File){const body=new FormData();body.append('file',file,file.name);const response=await fetch(`${API_URL}/decoration/hot-dates/bulk?year=${year}`,{method:'POST',headers:{Authorization:`Bearer ${token}`},body});return parseResponse<BulkUploadResult>(response,{notifyOnUnauthorized:true})}
...
export const deleteDecorationBooking=(token:string,id:string)=>authorizedRequest<{id:string}>(`/decoration/bookings/${encodeURIComponent(id)}`,token,{method:'DELETE'});
...
export const addDecorationFollowup=(token:string,id:string,payload:{date:string;nextDate?:string;note:string;closeInquiry?:boolean})=>authorizedRequest<DecorationBooking>(`/decoration/bookings/${id}/followups`,token,{method:'POST',body:JSON.stringify(payload)});
export const addDecorationPayment=(token:string,id:string,payload:import('./types').DecorationPaymentPayload)=>authorizedRequest<DecorationBooking>(`/decoration/bookings/${encodeURIComponent(id)}/payments`,token,{method:'POST',body:JSON.stringify(payload)});
...
export const fetchDecorationDashboardRecords=(token:string,type:import('./types').DecorationDashboardRecordType,page=1,limit=20)=>authorizedRequest<import('./types').DecorationDashboardRecords>(`/decoration/operations/dashboard-records?type=${encodeURIComponent(type)}&page=${page}&limit=${limit}`,token);
...
export const fetchDecorationAvailability=(token:string,id:string)=>authorizedRequest<import('./types').DecorationAvailabilityResult>(`/decoration/reservations/bookings/${encodeURIComponent(id)}/availability`,token);
...
export const createConfirmedDecorationBooking=(token:string,payload:Record<string,unknown>&import('./types').DecorationConfirmationPayload)=>authorizedRequest<import('./types').DecorationConfirmationResult>('/decoration/bookings/confirmed',token,{method:'POST',body:JSON.stringify(payload)});
export const saveDecorationSelection=(token:string,bookingId:string,items:Array<{itemId:string;quantity:number;position?:number;imageId?:string;description?:string}>,customItems:Array<{name:string;quantity:number;position?:number;description?:string;imageKey:string;imageUrl:string;displayMode?:import('./types').DecorationImageDisplayMode}>=[],generalNotes?:string)=>authorizedRequest<import('./types').DecorationSelectionResult>(`/decoration/reservations/bookings/${encodeURIComponent(bookingId)}`,token,{method:'PUT',body:JSON.stringify({items,customItems,generalNotes})});
export const fetchDecorationSelectionDraft=(token:string,bookingId:string,signal?:AbortSignal)=>authorizedRequest<import('./types').DecorationSelectionDraft|null>(`/decoration/selection-drafts/bookings/${encodeURIComponent(bookingId)}`,token,{signal});
export const saveDecorationSelectionDraft=(token:string,bookingId:string,payload:{revision:number;blocks:import('./types').DecorationDraftBlock[];generalNotes:string|null;finalPackagePrice:string},signal?:AbortSignal)=>authorizedRequest<import('./types').DecorationSelectionDraft>(`/decoration/selection-drafts/bookings/${encodeURIComponent(bookingId)}`,token,{method:'PUT',body:JSON.stringify(payload),signal});
export const deleteDecorationSelectionDraft=(token:string,bookingId:string)=>authorizedRequest<null>(`/decoration/selection-drafts/bookings/${encodeURIComponent(bookingId)}`,token,{method:'DELETE'});
export async function uploadCustomDecorationImage(token:string,bookingId:string,file:File,displayMode:import('./types').DecorationImageDisplayMode='COVER'){const body=new FormData();body.append('file',file);body.append('displayMode',displayMode);const response=await fetch(`${API_URL}/decoration/selection/bookings/${encodeURIComponent(bookingId)}/custom-images`,{method:'POST',headers:{Authorization:`Bearer ${token}`},body});return parseResponse<{key:string;url:string;mimeType:string;sizeBytes:number;displayMode:import('./types').DecorationImageDisplayMode}>(response,{notifyOnUnauthorized:true});}
...
export type DecorationReportFilters={month?:string;date?:string;from?:string;to?:string;status?:string;eventTypeId?:string;venueId?:string;hallId?:string;creatorId?:string;paymentState?:string;paymentMode?:string;search?:string;page?:string;limit?:string};
function decorationReportQuery(filters:DecorationReportFilters){const query=new URLSearchParams();Object.entries(filters).forEach(([key,value])=>{if(value?.trim())query.set(key,value.trim())});return query.toString()}
export const fetchDecorationReportSummary=(token:string,filters:DecorationReportFilters)=>authorizedRequest<import('./types').DecorationReportSummary>(`/decoration/reports/summary?${decorationReportQuery(filters)}`,token);
export const fetchDecorationReportBreakdown=(token:string,filters:DecorationReportFilters)=>authorizedRequest<import('./types').DecorationReportBreakdown>(`/decoration/reports/breakdown?${decorationReportQuery(filters)}`,token);
export const fetchDecorationRevenueAnalysis=(token:string,filters:DecorationReportFilters)=>authorizedRequest<import('./types').DecorationRevenueAnalysis>(`/decoration/reports/revenue?${decorationReportQuery(filters)}`,token);
export const fetchDecorationInventoryReport=(token:string,filters:DecorationReportFilters)=>authorizedRequest<import('./types').DecorationInventoryReport>(`/decoration/reports/inventory?${decorationReportQuery(filters)}`,token);
export const fetchDecorationEmployeePerformance=(token:string,filters:DecorationReportFilters)=>authorizedRequest<import('./types').DecorationEmployeePerformance>(`/decoration/reports/employees?${decorationReportQuery(filters)}`,token);
export const fetchDecorationReportEvents=(token:string,filters:DecorationReportFilters,page=1,limit=10)=>authorizedRequest<import('./types').DecorationReportEvents>(`/decoration/reports/events?${decorationReportQuery({...filters,page:String(page),limit:String(limit)})}`,token);
export async function downloadDecorationReport(token:string,filters:DecorationReportFilters,format:'csv'|'xlsx'){const response=await fetch(`${API_URL}/decoration/reports/export/${format}?${decorationReportQuery(filters)}`,{headers:{Authorization:`Bearer ${token}`}});if(!response.ok){const payload=await response.json().catch(()=>null) as {message?:string}|null;if(response.status===401)notifySessionExpired();throw new Error(payload?.message??'Unable to export decoration report.')}return response.blob()}
export const fetchDecorationPrintableReport=(token:string,filters:DecorationReportFilters)=>authorizedRequest<import('./types').DecorationPrintableReport>(`/decoration/reports/export/print?${decorationReportQuery(filters)}`,token);
export const fetchDecorationLimitedReport=(token:string,kind:import('./types').DecorationLimitedReportKind,filters:DecorationReportFilters)=>authorizedRequest<import('./types').DecorationLimitedReport>(`/decoration/reports/${kind==='booking'?'bookings':kind==='advance'?'advances':kind}?${decorationReportQuery(filters)}`,token);
export const updateDecorationWorksheetRow=(token:string,id:string,payload:Record<string,unknown>)=>authorizedRequest<import('./types').DecorationBooking>(`/decoration/bookings/${encodeURIComponent(id)}/worksheet`,token,{method:'PATCH',body:JSON.stringify(payload)});
export async function downloadDecorationLimitedReport(token:string,kind:import('./types').DecorationLimitedReportKind,filters:DecorationReportFilters,format:'csv'|'xlsx'){const response=await fetch(`${API_URL}/decoration/reports/export/${kind}/${format}?${decorationReportQuery(filters)}`,{headers:{Authorization:`Bearer ${token}`}});if(!response.ok){const payload=await response.json().catch(()=>null) as {message?:string|string[]}|null;if(response.status===401)notifySessionExpired();throw new Error(Array.isArray(payload?.message)?payload.message.join(', '):payload?.message??'Unable to export decoration report.')}return response.blob()}
export const fetchDecorationLimitedPrintableReport=(token:string,kind:import('./types').DecorationLimitedReportKind,filters:DecorationReportFilters)=>authorizedRequest<import('./types').DecorationLimitedPrintableReport>(`/decoration/reports/export/${kind}/print?${decorationReportQuery(filters)}`,token);
