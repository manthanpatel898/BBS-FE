...
export const saveDecorationSelection=(token:string,bookingId:string,items:Array<{itemId:string;quantity:number;imageId?:string;description?:string}>,finalPackagePrice:number,customItems:Array<{name:string;quantity:number;description?:string;imageKey:string;imageUrl:string}>=[])=>authorizedRequest<import('./types').DecorationSelectionResult>(`/decoration/reservations/bookings/${encodeURIComponent(bookingId)}`,token,{method:'PUT',body:JSON.stringify({items,customItems,finalPackagePrice})});
