...
- Both upload owners reset their mounted guard during effect setup, so React Strict Mode's setup-cleanup-setup replay cannot permanently suppress async selection or confirmation.
...
- A real `React.StrictMode` regression proves async selection opens and confirms crop for both `ItemModal` and `CatalogItemCard` after effect replay.
- Focused command: `node --import tsx --test --test-reporter=spec lib/decoration/catalog-crop-integration.behavior.test.tsx`.

## Verification

- Task 7's crop modal now tracks the active file from effects rather than reading/writing its stale-export ref during render. Its six behavioral tests remain green, and the test-only third-party adapter uses one narrowly documented lint suppression.
- All 118 decoration tests, TypeScript, full ESLint (zero errors; one unrelated warning), and the Next.js static build (46 pages) passed after the review fixes.
