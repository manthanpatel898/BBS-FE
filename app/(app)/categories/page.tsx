...
} from '@/lib/categories/category-sync';
...
export default function CategoriesPage() {
  ...
  async function handleSyncPreview(event: { preventDefault(): void }) {
    event.preventDefault();
    if (!syncFile || !accessToken) return;
    if (!effectiveRestaurantId) {
      dispatchSync({ type: 'REQUEST_FAILED', message: 'Select a restaurant first.' });
      return;
    }

    dispatchSync({ type: 'UPLOAD_STARTED', fileName: syncFile.name });
    try {
      const preview = await previewCategorySync(
        accessToken,
        syncFile,
      ...
      dispatchSync({
        ...
        message:
          requestError instanceof Error
            ? requestError.message
            : 'Unable to preview category changes.',
      });
    }
  }
  ...
  async function handleSyncDownload(format: 'csv' | 'xlsx') {
    ...
    try {
      ...
      const blob = await downloadCategorySyncExport(
        accessToken,
        format,
        isSuperAdmin ? effectiveRestaurantId : undefined,
      );
      downloadFile(blob, `categories-current.${format}`, blob.type);
    } catch (requestError) {
      dispatchSync({
        type: 'REQUEST_FAILED',
        message:
          requestError instanceof Error
            ? requestError.message
            : 'Unable to download category data.',
      });
    } finally {
  ...
  async function handleSyncConfirm() {
    if (!accessToken || !syncState.preview || !canConfirmCategorySync(syncState)) {
      return;
    }
    setIsSyncConfirmOpen(false);
    dispatchSync({ type: 'CONFIRM_STARTED' });
    try {
      const result = await confirmCategorySync(
        accessToken,
        syncState.preview.previewId,
        isSuperAdmin ? effectiveRestaurantId : undefined,
      );
      dispatchSync({ type: 'CONFIRM_SUCCEEDED', result });
      setSuccessMessage('Categories synchronized successfully.');
      setPage(1);
      ...
      dispatchSync({
        ...
        message:
          requestError instanceof Error
            ? requestError.message
            : 'Unable to synchronize categories.',
      });
    }
  }
  ...
  return (
    <ConfigRoute>
      <section className="space-y-5">
        ...
        {isSyncOpen ? (
          <CommonModal
            title="Export & Sync Categories"
            description="Download the restaurant's current data, edit it, then preview every change before applying it."
          ...
          >
            <div className="space-y-5">
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                <p className="text-xs font-semibold uppercase tracking-wider text-slate-500">
                  1. Download current category data
                </p>
              ...
              <div className="space-y-1 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
                <p className="font-semibold">The uploaded file is treated as the complete category list.</p>
                <p>Rows with an existing categoryId update that category. Blank IDs create new categories.</p>
                <p>Active categories missing from the file will be deactivated after confirmation, never deleted.</p>
              </div>
              ...
              <form onSubmit={(event) => void handleSyncPreview(event)} className="space-y-4">
                ...
                {syncState.preview ? (
                  <CategorySyncPreviewPanel preview={syncState.preview} />
                ) : null}

                {syncState.phase === 'success' && syncState.result ? (
                  <div className="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-semibold text-emerald-800">
                    Categories synchronized successfully. The category list has been refreshed.
                  </div>
                ) : null}
                ...
                <div className="grid gap-2 border-t border-slate-200 pt-4 sm:flex sm:justify-end">
                  ...
                  {syncState.preview ? (
                    <LoadingButton
                      ...
                      disabled={!canConfirmCategorySync(syncState)}
        ...
        {isSyncConfirmOpen && syncState.preview ? (
          <ConfirmModal
            ...
            message={`This will create ${syncState.preview.summary.create}, update ${syncState.preview.summary.update}, reactivate ${syncState.preview.summary.reactivate}, and deactivate ${syncState.preview.summary.deactivate} categories. Historical bookings will not change.`}
