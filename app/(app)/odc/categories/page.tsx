...
import { OdcCategory, OdcMenu, Restaurant } from '@/lib/auth/types';
...
export default function OdcCategoriesPage() {
  useAppPageHeader({
    eyebrow: 'Outdoor Catering',
    title: 'ODC Categories',
  });
  ...
  const [editingCategory, setEditingCategory] = useState<OdcCategory | null>(null);
  const [formState, setFormState] = useState<CategoryFormState>(initialFormState);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [categoryToDelete, setCategoryToDelete] = useState<OdcCategory | null>(null);
    ...
    async function loadData() {
      try {
        ...
        setError(
          requestError instanceof Error
            ? requestError.message
            : 'Unable to fetch ODC categories.',
        );
      } finally {
        setIsLoading(false);
      }
  ...
  async function reloadCategories(token: string, nextPage: number) {
    ...
    const response = await fetchOdcCategories(token, {
      page: nextPage,
      limit,
      search,
      ...(isSuperAdmin ? { restaurantId: effectiveRestaurantId } : {}),
  ...
  function openEditModal(category: OdcCategory) {
  ...
  function toggleRule(menu: OdcMenu, sectionTitle: string) {
  ...
  function addRule(menu: OdcMenu, sectionTitle: string) {
    const key = makeRuleKey(menu.id, sectionTitle);
    const exists = formState.menuRules.some(
      (rule) => makeRuleKey(rule.menuId, rule.sectionTitle) === key,
    );
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const pricePerPlate = Number(formState.pricePerPlate);
    if (!Number.isFinite(pricePerPlate) || pricePerPlate < 0) {
      setError('Price per plate must be a valid amount.');
      return;
    }
    ...
    try {
      ...
      if (editingCategory) {
        await updateOdcCategory(accessToken, editingCategory.id, payload);
        setSuccessMessage('ODC category updated successfully.');
      } else {
        await createOdcCategory(accessToken, payload);
        setSuccessMessage('ODC category created successfully.');
      }

      setIsModalOpen(false);
      const nextPage = editingCategory ? page : 1;
      setPage(nextPage);
      await reloadCategories(accessToken, nextPage);
    } catch (requestError) {
      setError(
        requestError instanceof Error
          ? requestError.message
          : 'Unable to save ODC category.',
      );
    } finally {
  ...
  async function handleDelete() {
    ...
    try {
      setIsDeleting(true);
      setError('');
      await deleteOdcCategory(accessToken, categoryToDelete.id);
      setSuccessMessage('ODC category deleted successfully.');
  ...
  const configuredVisibleItemsCount = formState.menuRules.reduce(
    (total, rule) => total + rule.allowedItems.length,
    0,
  );
  const configuredTotalItemsCount = groupedMenus.reduce(
    (total, menu) =>
      total + menu.visibleSections.reduce((count, section) => count + section.items.length, 0),
    0,
  );
      ...
      <section className="space-y-5">
                  ...
                  <tr key={category.id} className="border-t border-slate-100 hover:bg-slate-50/60">
                    ...
                    <td className="px-5 py-4">
                      <div className="flex gap-2">
                        <button type="button" onClick={() => openEditModal(category)} className="rounded-xl border border-slate-200 px-3 py-2 text-sm font-medium text-slate-600 transition hover:bg-slate-50">Edit</button>
                        <button type="button" onClick={() => setCategoryToDelete(category)} className="rounded-xl border border-red-200 px-3 py-2 text-sm font-medium text-red-600 transition hover:bg-red-50">Delete</button>
                      </div>
                    </td>
                  </tr>
                ))
        ...
        <div className="space-y-3 md:hidden">
          {isLoading ? (
            ...
            categories.map((category) => (
              <article key={`mobile-${category.id}`} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                ...
                <div className="mt-4 flex gap-2">
                  <button type="button" onClick={() => openEditModal(category)} className="flex-1 rounded-xl border border-slate-200 px-3 py-2 text-sm font-medium text-slate-600">Edit</button>
                  <button type="button" onClick={() => setCategoryToDelete(category)} className="flex-1 rounded-xl border border-red-200 px-3 py-2 text-sm font-medium text-red-600">Delete</button>
                </div>
              </article>
            ))
          )}
        ...
        {categoryToDelete ? (
          <ConfirmModal
            ...
            message="This removes the ODC category only. Banquet categories are not affected."
            confirmLabel="Delete"
        ...
        {isItemPickerOpen ? (
          <CommonModal
            ...
            description="Choose one menu item. You can configure visible subitems and count after adding it."
          ...
          >
            <div className="space-y-4">
              {availableSections.length === 0 ? (
                <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50 px-4 py-8 text-center text-sm text-slate-500">
                  All available menu items are already added to this category.
                </div>
              ) : (
                <div className="grid gap-3 sm:grid-cols-2">
                  {availableSections.map(({ menu, section }) => {
                    const showMenuTitle =
                      menu.title?.trim().toLowerCase() !== section.sectionTitle.trim().toLowerCase();
                    ...
                    return (
                      <button
                        ...
                        className="rounded-2xl border border-slate-200 bg-white p-4 text-left transition hover:border-amber-300 hover:bg-amber-50/70"
                      >
                        {showMenuTitle ? (
                          <p className="text-xs font-semibold uppercase tracking-wider text-slate-500">
                            {menu.title || 'Untitled menu'}
                        ...
                        <h4 className={`${showMenuTitle ? 'mt-1' : ''} text-base font-semibold text-slate-900`}>
