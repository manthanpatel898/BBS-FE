...
export default function DecorationReportsPage(){
  ...
  return <div className="space-y-8">
    <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><p className="text-xs font-semibold uppercase tracking-[.25em] text-amber-600">Event Decoration · Reports</p><h1 className="mt-2 text-2xl font-bold text-slate-900 sm:text-3xl">All Reports</h1><p className="mt-2 max-w-2xl text-sm text-slate-500">Four focused reports for daily event operations, booking reviews and payment collection. Apply filters, preview records, then export as CSV or XLSX.</p></section>
    {(['Bookings','Financial'] as const).map(group=><section key={group} className="space-y-4"><div><p className={`text-xs font-semibold uppercase tracking-[.2em] ${group==='Financial'?'text-emerald-600':'text-amber-600'}`}>{group==='Financial'?'Money & Collections':'Inquiry & Confirmations'}</p><h2 className="mt-1 text-xl font-bold text-slate-900">{group}</h2></div><div className="grid gap-4 md:grid-cols-2">{REPORTS.filter(item=>item.group===group).map(item=><Link key={item.type} href={`/decoration/reports/view/?type=${item.type}`} className={`group rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md ${group==='Financial'?'hover:border-emerald-200':'hover:border-amber-200'}`}><div className="flex items-start justify-between gap-4"><div><h3 className="text-lg font-bold text-slate-900">{item.title}</h3><p className="mt-2 text-sm leading-6 text-slate-500">{item.description}</p></div><span className="text-xl text-slate-300 transition group-hover:translate-x-1">→</span></div><span className={`mt-4 inline-flex rounded-full px-3 py-1 text-xs font-semibold ${group==='Financial'?'bg-emerald-50 text-emerald-700':'bg-amber-50 text-amber-700'}`}>{item.badge}</span></Link>)}</div></section>)}
  </div>;
}
