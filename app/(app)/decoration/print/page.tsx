...
export default function Page() {
  ...
  const mode = params.get('mode') === 'internal' ? 'internal' : 'customer';
  const { accessToken, user } = useAuth();
  const [booking, setBooking] = useState<DecorationBooking | null>(null);
  const [error, setError] = useState('');
  useEffect(() => { if (!accessToken || !bookingId) return; let active = true; fetchDecorationBooking(accessToken, bookingId).then((value) => { if (active) setBooking(value); }).catch((reason) => { if (active) setError(reason instanceof Error ? reason.message : 'Unable to load print view.'); }); return () => { active = false; }; }, [accessToken, bookingId]);
  ...
  return <main className="mx-auto min-h-screen max-w-5xl bg-white p-4 text-slate-900 sm:p-8 print:max-w-none print:p-0">
    <div className="mb-6 flex flex-wrap items-center justify-between gap-3 rounded-xl bg-slate-100 p-3 print:hidden"><div><p className="text-sm font-bold">{mode === 'internal' ? 'Internal operational output' : 'Customer visual output'}</p><p className="text-xs text-slate-500">Choose “Save as PDF” in the print dialog to download a PDF.</p></div><div className="flex flex-wrap gap-2"><Link href={`/decoration/print?bookingId=${encodeURIComponent(booking.id)}&mode=${mode === 'internal' ? 'customer' : 'internal'}`} className="rounded-lg border bg-white px-3 py-2 text-sm font-semibold">Switch to {mode === 'internal' ? 'customer' : 'internal'}</Link><button type="button" onClick={() => window.print()} className="rounded-lg bg-slate-900 px-4 py-2 text-sm font-semibold text-white">Download PDF / Print</button><Link href={`/decoration/event-detail?bookingId=${encodeURIComponent(booking.id)}`} className="rounded-lg border bg-white px-3 py-2 text-sm font-semibold">Back</Link></div></div>
    ...
    <section className="mb-8 grid gap-5 rounded-2xl border p-5 sm:grid-cols-2 print:grid-cols-2"><Details title="Customer" rows={[["Name", booking.customer.name], ["Mobile", booking.customer.mobile]]} /><Details title="Event & venue" rows={[["Date", booking.startDate === booking.endDate ? formatDate(booking.startDate) : `${formatDate(booking.startDate)} – ${formatDate(booking.endDate)}`], ["Time", `${booking.startTime} – ${booking.endTime}`], ["Venue", booking.venue.name], ["Hall", booking.hall?.name || 'Not applicable'], ["Address", booking.address || 'Not provided']]} />{mode === 'internal' && <><Details title="Payment" rows={[["Package", money(booking.packageRate)], ["Collected", money(booking.totalCollected)], ["Outstanding", money(booking.outstandingAmount)]]} /><Details title="Operations" rows={[["Status", booking.status.replaceAll('_', ' ')], ["Slot", booking.timeSlot], ["Notes", booking.notes || 'None']]} /></>}</section>
    <section><div className="mb-5"><h2 className="text-2xl font-bold">Selected Decoration</h2><p className="mt-1 text-sm text-slate-500">{booking.decorationSnapshot?.length ?? 0} selected items</p></div><DecorationSnapshotGallery lines={booking.decorationSnapshot ?? []} printable internal={mode === 'internal'} /></section>
