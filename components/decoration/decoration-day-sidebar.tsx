...
export function DecorationDaySidebar({ date, bookings, canAdd, onClose, onAdd, onOpenBooking }: {
  date: string;
    ...
    <div className="fixed inset-0 z-40 bg-slate-950/45" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <aside role="dialog" aria-modal="true" aria-label={`Events on ${label}`} className="absolute inset-y-0 left-0 flex w-full max-w-xl flex-col border-r border-slate-200 bg-white shadow-2xl lg:max-w-5xl">
        <header className="border-b border-slate-200 bg-white px-5 py-5 sm:px-7">
          ...
          {canAdd ? <button type="button" onClick={onAdd} className="mt-5 w-full rounded-xl bg-amber-500 px-4 py-3 font-bold text-white shadow-sm hover:bg-amber-600 sm:w-auto">Add inquiry</button> : null}
        </header>
        <div className="flex-1 overflow-y-auto p-4 sm:p-7">
          {bookings.length ? <div className="grid gap-4 md:grid-cols-2">{bookings.map((booking) => (
            <button type="button" key={booking.id} onClick={() => onOpenBooking(booking.id)} className={`w-full rounded-2xl border p-4 text-left transition hover:-translate-y-0.5 focus:outline-none focus:ring-2 focus:ring-amber-200 ${getDecorationStatusMeta(booking.status).cardClass}`}>
              ...
              <dl className="mt-4 space-y-2 text-sm"><div><dt className="inline font-bold text-slate-700">Location: </dt><dd className="inline text-slate-600">{booking.venue.name}{booking.hall ? ` / ${booking.hall.name}` : ''}</dd></div><div><dt className="inline font-bold text-slate-700">Time: </dt><dd className="inline text-slate-600">{formatDecorationTimeRange(booking.startTime,booking.endTime)} · {booking.timeSlot}</dd></div></dl>
              <div className="mt-4 grid grid-cols-3 gap-2 border-t border-slate-100 pt-4 text-xs"><div><p className="text-slate-500">Package</p><p className="mt-1 break-words font-bold text-slate-900">{booking.isPackagePriceFinalized?formatIndianCurrency(booking.packageRate):'Not finalized'}</p></div><div><p className="text-slate-500">Received</p><p className="mt-1 font-bold text-emerald-700">{formatIndianCurrency(booking.totalCollected)}</p></div><div><p className="text-slate-500">Pending</p><p className="mt-1 break-words font-bold text-red-700">{booking.isPackagePriceFinalized?formatIndianCurrency(booking.outstandingAmount):'Not finalized'}</p></div></div>
            </button>
          ))}</div> : <div className="rounded-2xl border border-dashed border-slate-300 bg-white px-5 py-12 text-center"><h3 className="font-bold text-slate-900">No events on this date</h3><p className="mt-2 text-sm text-slate-600">{canAdd ? 'Create an inquiry to start planning this date.' : 'No events were recorded on this date.'}</p></div>}
        </div>
      </aside>
    </div>
  );
