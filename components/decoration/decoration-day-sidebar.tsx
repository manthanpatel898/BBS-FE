...
export function DecorationDaySidebar({ date, bookings, onClose, onAdd, onOpenBooking }: {
  ...
  return (
    <div className="fixed inset-0 z-40 bg-slate-950/45" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <aside role="dialog" aria-modal="true" aria-label={`Events on ${label}`} className="absolute inset-y-0 left-0 flex w-full flex-col bg-slate-50 shadow-2xl sm:max-w-2xl lg:max-w-4xl">
        <header className="border-b border-slate-200 bg-white px-5 py-5 sm:px-7">
          <div className="flex items-start justify-between gap-4"><div><h2 className="text-2xl font-black text-slate-950">{label}</h2><p className="mt-1 text-sm font-semibold text-slate-500">{bookings.length} event{bookings.length === 1 ? '' : 's'}</p></div><button type="button" onClick={onClose} aria-label="Close selected date" className="flex h-11 w-11 items-center justify-center rounded-full border border-slate-200 text-xl text-slate-600 hover:bg-slate-50">×</button></div>
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-7">
          {bookings.length ? <div className="grid gap-4 md:grid-cols-2">{bookings.map((booking) => (
            <button type="button" key={booking.id} onClick={() => onOpenBooking(booking.id)} className="rounded-2xl border border-slate-200 bg-white p-5 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-amber-300 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-amber-500">
