...
export function DecorationEventDetailModal({ bookingId, initialBooking, onClose, onUpdated }: {
  ...
  const content = loading && !booking
    ...
    : !booking
      ...
      : <Detail booking={booking} warning={error} onEdit={() => setEditing(true)} onConfirm={() => setConfirming(true)} />;
  ...
  return (
    <div className={onClose ? 'fixed inset-0 z-50 overflow-y-auto bg-slate-900/40 p-0 backdrop-blur-sm sm:p-5' : ''} role={onClose ? 'dialog' : undefined} aria-modal={onClose ? true : undefined} aria-label={onClose ? 'Decoration event details' : undefined} onMouseDown={(event) => { if (onClose && event.target === event.currentTarget) onClose(); }}>
      ...
      {confirming && booking ? <DecorationConfirmationModal booking={booking} onClose={() => setConfirming(false)} onConfirmed={(updated) => { setBooking(updated); onUpdated?.(updated); setConfirming(false); }} /> : null}
    </div>
  );
}

function Detail({ booking, warning, onEdit, onConfirm }: { booking: DecorationBooking; warning: string; onEdit: () => void; onConfirm: () => void }) {
  const activities = [
  ...
  return <section className="mx-auto max-w-7xl space-y-5 p-4 pb-24 pt-20 sm:p-6 sm:pt-16 lg:p-8 lg:pt-16">
    ...
    <div className="fixed inset-x-0 bottom-0 z-[55] border-t bg-white/95 p-3 shadow-2xl backdrop-blur sm:hidden"><div className="flex gap-2 overflow-x-auto"><Actions booking={booking} onEdit={onEdit} onConfirm={onConfirm} /></div></div>
  </section>;
}

function Actions({ booking, onEdit, onConfirm }: { booking: DecorationBooking; onEdit: () => void; onConfirm: () => void }) {
  const { user } = useAuth();
  const canConfirm = user?.role === 'company_admin' || hasPermission(user, PERMISSIONS.DECORATION_BOOKINGS_CONFIRM);
  const canSelect = !['INQUIRY', 'CANCELLED', 'CLOSED_INQUIRY'].includes(booking.status);
  return <><button type="button" onClick={onEdit} className="shrink-0 rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm font-semibold text-slate-900">Edit Inquiry</button>{booking.status === 'INQUIRY' && canConfirm ? <button type="button" onClick={onConfirm} className="shrink-0 rounded-xl bg-emerald-600 px-4 py-2.5 text-sm font-bold text-white hover:bg-emerald-700">Confirm Booking</button> : null}{canSelect ? <Link href={`/decoration/selection?bookingId=${encodeURIComponent(booking.id)}`} className="shrink-0 rounded-xl bg-amber-500 px-4 py-2.5 text-sm font-semibold text-slate-950">{booking.decorationSnapshot?.length ? 'Edit Selection' : 'Choose Decoration'}</Link> : null}<Link href={`/decoration/print?bookingId=${encodeURIComponent(booking.id)}&mode=customer`} className="shrink-0 rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm font-semibold text-slate-900">Customer PDF</Link></>;
}
