...
export function DecorationDashboard(){
  ...
  if(query.view)return <div className="min-w-0 max-w-full text-slate-900">
    ...
    {query.bookingId?<DecorationEventDetailModal bookingId={query.bookingId} initialBooking={records?.items.find(item=>item.id===query.bookingId)} onClose={()=>router.replace(decorationDashboardUrl({view:query.view,page:query.page}),{scroll:false})} onUpdated={(updated)=>setRecords(current=>current?{...current,items:current.items.map(item=>item.id===updated.id?updated:item)}:current)} onDeleted={(deletedId)=>setRecords(current=>current?{...current,items:current.items.filter(item=>item.id!==deletedId),pagination:{...current.pagination,total:Math.max(0,current.pagination.total-1)}}:current)}/>:null}
  </div>;
