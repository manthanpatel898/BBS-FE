...
function StaffEntryForm({ questions, busy, pendingLink, onCancel, onSubmit }: { questions: BookingFeedbackQuestionSnapshot[]; busy: boolean; pendingLink: boolean; onCancel: () => void; onSubmit: (input: { answers: Array<{ questionId: string; rating: number }>; comment: string; captureMethod: BookingFeedbackCaptureMethod; captureMethodOther: string; staffContext: string; consentConfirmed: true }) => Promise<void> }) {
  ...
  return <section className="mt-6"><div className="rounded-2xl border border-indigo-200 bg-indigo-50 p-4"><p className="font-bold text-indigo-950">Record feedback with the customer</p><p className="mt-1 text-sm leading-6 text-indigo-800">This response will be marked as entered by staff and your name will be recorded for audit purposes.</p>{pendingLink ? <p className="mt-2 text-sm font-bold text-red-700">Submitting here will make the previously generated customer link unusable.</p> : null}</div><div className="mt-4 flex items-center justify-between gap-3"><h3 className="font-black text-slate-950">Feedback questions</h3><span className="shrink-0 rounded-full bg-slate-100 px-3 py-1 text-xs font-bold text-slate-700">{answered}/{questions.length} answered</span></div><div className="mt-3 space-y-3">{questions.map((question, index) => <div key={question.id} className="rounded-2xl border border-slate-200 p-4"><p className="text-sm font-bold leading-6 text-slate-900"><span className="mr-2 text-amber-600">{index + 1}.</span>{question.text}</p><div role="radiogroup" aria-label={`Rating for ${question.text}`} className="mt-3 grid grid-cols-5 gap-2">{[1,2,3,4,5].map((rating) => <button type="button" role="radio" aria-checked={ratings[question.id] === rating} aria-label={`${rating} stars`} key={rating} onClick={() => setRatings((values) => ({ ...values, [question.id]: rating }))} className={`min-h-12 rounded-xl border font-black ring-offset-2 transition focus-visible:ring-2 focus-visible:ring-amber-500 ${rating <= (ratings[question.id] || 0) ? 'border-amber-400 bg-amber-100 text-amber-700 shadow-sm' : 'border-slate-200 bg-slate-50 text-slate-400'}`}>★<span className="block text-[10px] text-slate-700">{rating}</span></button>)}</div></div>)}</div><div className="mt-4 grid gap-4 rounded-2xl border border-slate-200 bg-slate-50/50 p-4 sm:grid-cols-2"><label className="text-sm font-bold text-slate-800">How was feedback captured?<select value={captureMethod} onChange={(event) => setCaptureMethod(event.target.value as BookingFeedbackCaptureMethod | '')} className={`${FEEDBACK_CONTROL_CLASS} mt-2 min-h-12 w-full rounded-xl border border-slate-200 p-3 font-normal outline-none focus:border-amber-400 focus:ring-4 focus:ring-amber-100`}><option value="">Select method</option><option value="IN_PERSON">In person</option><option value="PHONE">Phone</option><option value="WHATSAPP">WhatsApp</option><option value="EMAIL">Email</option><option value="OTHER">Other</option></select></label>{captureMethod === 'OTHER' ? <label className="text-sm font-bold text-slate-800">Describe the method<input value={captureMethodOther} maxLength={100} onChange={(event) => setCaptureMethodOther(event.target.value)} className={`${FEEDBACK_CONTROL_CLASS} mt-2 min-h-12 w-full rounded-xl border border-slate-200 p-3 font-normal outline-none focus:border-amber-400 focus:ring-4 focus:ring-amber-100`}/></label> : null}</div><label className="mt-4 block text-sm font-bold text-slate-800">Customer comment <span className="font-normal text-slate-500">(optional)</span><textarea value={comment} maxLength={2000} onChange={(event) => setComment(event.target.value)} className={`${FEEDBACK_CONTROL_CLASS} mt-2 min-h-24 w-full rounded-xl border border-slate-200 p-3 font-normal outline-none focus:border-amber-400 focus:ring-4 focus:ring-amber-100`}/></label><label className="mt-4 block text-sm font-bold text-slate-800">Staff context <span className="font-normal text-slate-500">(optional · internal only)</span><textarea value={staffContext} maxLength={2000} onChange={(event) => setStaffContext(event.target.value)} className={`${FEEDBACK_CONTROL_CLASS} mt-2 min-h-20 w-full rounded-xl border border-slate-200 p-3 font-normal outline-none focus:border-amber-400 focus:ring-4 focus:ring-amber-100`}/></label><label className="mt-4 flex items-start gap-3 rounded-xl bg-slate-50 p-4 text-sm font-semibold text-slate-800"><input type="checkbox" checked={consentConfirmed} onChange={(event) => setConsentConfirmed(event.target.checked)} className="mt-1 h-5 w-5 shrink-0 accent-indigo-600"/><span>I confirm that the customer supplied or approved this feedback.</span></label>{error ? <p role="alert" className="mt-3 rounded-xl border border-red-200 bg-red-50 p-3 text-sm font-semibold text-red-700">{error}</p> : null}<div className="mt-5 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><button type="button" disabled={busy} onClick={onCancel} className="rounded-xl border border-slate-300 bg-white px-5 py-3 font-bold text-slate-700">Cancel</button><button type="button" disabled={busy || !questions.length} onClick={() => void submit()} className="rounded-xl bg-indigo-600 px-5 py-3 font-bold text-white disabled:opacity-50">{busy ? 'Saving feedback…' : 'Save feedback'}</button></div></section>;
}
...
export function BookingFeedbackModal({ accessToken, orderId, customerName, eventType, eventDate, onClose }: Props) {
  ...
  return <div data-feedback-surface="true" className="fixed inset-0 z-[100] flex items-end justify-center bg-slate-950/55 backdrop-blur-sm sm:items-center sm:p-6" role="dialog" aria-modal="true" aria-labelledby="booking-feedback-title">
